...
```bash
larakube php artisan wayfinder:generate --no-interaction
```
For form helpers, use `--with-form` flag:
```bash
larakube php artisan wayfinder:generate --with-form --no-interaction
```
...
1. Run `larakube php artisan wayfinder:generate` to regenerate routes if Vite plugin isn't installed
