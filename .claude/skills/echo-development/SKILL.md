...
```bash
larakube php artisan install:broadcasting
```
...
```bash
larakube php artisan make:event OrderShipped
```
...
```bash
larakube php artisan make:channel OrderChannel
```

List all registered channels:

```bash
larakube php artisan channel:list
```
...
```bash
larakube php artisan queue:work    # Required for ShouldBroadcast events

larakube php artisan reverb:start  # Required for Reverb driver
...
- Channel classes via `larakube php artisan make:channel` — use for complex authorization logic that benefits from dependency injection or reusable logic across channels.
...
- Reverb is long-running: Code changes require `larakube php artisan reverb:restart`.
