...
```bash
larakube php artisan horizon:install
```
...
1. Run `larakube php artisan horizon` and visit `/horizon`
...
- The metrics dashboard stays blank until `horizon:snapshot` is scheduled. Running `larakube php artisan horizon` alone does not populate metrics.
